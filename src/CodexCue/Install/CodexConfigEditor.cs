...
namespace CodexCue.Install {
    public static class CodexConfigEditor {
        private const string Section = "[plugins.\"codex-cue@personal\".mcp_servers.codex_cue]";
        private const string LegacySection = "[plugins.\"codex-option-prompts@personal\".mcp_servers.codex_option_prompts]";
        ...
        public static string EnableDirectQuestions(string source) {
            ...
            RemoveSection(lines, LegacySection);
        ...
        private static void RemoveSection(IList<string> lines, string section) {
            for (int index = 0; index < lines.Count; index++) {
                if (!String.Equals(lines[index].Trim(), section, StringComparison.Ordinal)) continue;
                int end = FindNextSection(lines, index + 1);
                while (end > index) { lines.RemoveAt(index); end--; }
                while (index < lines.Count && String.IsNullOrWhiteSpace(lines[index])) lines.RemoveAt(index);
                return;
            }
        }
    }
