...
namespace CodexCue.Hooks {
    public static class HookContextWriter {
        ...
        public const string CompletionPolicy =
            "Mandatory completed-task follow-up policy: Whenever the current user-requested task is genuinely complete, use the next-step-options skill before ending the turn. Generate exactly 2 or 3 relevant next-task choices and call ask_options from codex_cue so the user can click one. Do not merely list the choices in assistant prose. Do not run this workflow for incomplete work, blockers, greetings, status-only replies, or when the user explicitly opted out of follow-up suggestions.";

        public const string CompletionContinuationReason =
            "Before ending this turn, check whether the user's requested task is genuinely complete. If required work remains, continue it. If the task is complete and clickable follow-up choices have not already been handled for this completion, use $next-step-options now: create exactly 2 or 3 relevant next tasks and call codex_cue.ask_options. Do not merely list them in prose. If the follow-up tool was already handled, the task is blocked or incomplete, this is only a greeting/status reply, or the user explicitly opted out, finish without another follow-up prompt.";
