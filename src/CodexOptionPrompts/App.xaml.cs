...
namespace CodexOptionPrompts {
    public partial class App : System.Windows.Application {
        ...
        protected override void OnStartup(System.Windows.StartupEventArgs e) {
            base.OnStartup(e);
            CodexOptionPrompts.Application.AppModeOptions options;
            try {
                options = CodexOptionPrompts.Application.AppModeParser.Parse(e.Args);
            ...
            if (options.Mode == CodexOptionPrompts.Application.AppMode.Mcp) {
                RunMcp();
                Shutdown(0);
            } else if (options.Mode == CodexOptionPrompts.Application.AppMode.Hook) {
                RunHook(options.HookEvent);
                Shutdown(0);
            } else if (options.Mode == CodexOptionPrompts.Application.AppMode.Host) {
                StartHost();
            } else if (options.Mode == CodexOptionPrompts.Application.AppMode.Demo) {
                StartDemo(options.Automation);
            } else if (options.Mode == CodexOptionPrompts.Application.AppMode.InstallPlugin) {
                RunInstall(options);
            } else if (options.Mode == CodexOptionPrompts.Application.AppMode.UninstallPlugin) {
        ...
        private void RunInstall(CodexOptionPrompts.Application.AppModeOptions options) {
            try {
                ...
                string stagedPlugin = Path.Combine(directory, "plugin", "codex-option-prompts");
                if (!Directory.Exists(stagedPlugin)) stagedPlugin = Path.Combine(directory, "plugins", "codex-option-prompts");
                ...
                if (String.IsNullOrWhiteSpace(options.TestHome)) {
                    string installedProgram = Path.Combine(paths.ProgramDirectory, "CodexOptionPrompts.exe");
        ...
        private void RunUninstall(CodexOptionPrompts.Application.AppModeOptions options) {
            try {
                InstallPaths paths = String.IsNullOrWhiteSpace(options.TestHome) ? InstallPaths.Current() : InstallPaths.ForHome(options.TestHome);
                new InstallerEntry(paths).Uninstall(new UninstallRequest { RestoreBackup = true });
                if (String.IsNullOrWhiteSpace(options.TestHome)) {
                    StartupRegistration.Disable(Path.Combine(paths.ProgramDirectory, "CodexOptionPrompts.exe"));
                }
                Shutdown(0);
            } catch (InstallException error) {
                Console.Error.WriteLine(error.Code + ": " + error.Message);
        ...
        private void StartDemo(bool automation) {
            ...
            bool referenceCapture = String.Equals(
                Environment.GetEnvironmentVariable("CODEX_OPTION_PROMPTS_REFERENCE_CAPTURE"), "1", StringComparison.Ordinal);
