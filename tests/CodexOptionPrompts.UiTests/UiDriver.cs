...
namespace CodexOptionPrompts.UiTests {
    internal sealed class UiDriver : IDisposable {
        ...
        private static UiDriver Start(string arguments, double scale, bool referenceCapture) {
            string executable = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "CodexOptionPrompts.exe");
            ...
            start.EnvironmentVariables["CODEX_OPTION_PROMPTS_AUTOMATION_SCALE"] = scale.ToString(System.Globalization.CultureInfo.InvariantCulture);
            if (referenceCapture) start.EnvironmentVariables["CODEX_OPTION_PROMPTS_REFERENCE_CAPTURE"] = "1";
