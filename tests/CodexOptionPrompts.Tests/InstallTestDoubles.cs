...
namespace CodexOptionPrompts.Tests {
    internal sealed class TemporaryHome : IDisposable {
        ...
        private TemporaryHome(string path) {
            ...
            StagedProgramPath = System.IO.Path.Combine(StagingDirectory, "CodexOptionPrompts.exe");
        ...
        public static TemporaryHome WithPrototype() {
            string root = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "CodexOptionPrompts.Tests", Guid.NewGuid().ToString("N"));
            ...
            Write(home.Paths.PluginDirectory, ".codex-plugin\\plugin.json", "{\"name\":\"codex-option-prompts\",\"version\":\"0.1.0\"}");
            ...
            Write(home.StagedPluginPath, ".codex-plugin\\plugin.json", "{\"name\":\"codex-option-prompts\",\"version\":\"1.0.0\"}");
            Write(home.StagedPluginPath, "skills\\option-prompts\\SKILL.md", "final managed skill");
        ...
        public bool PrototypeMarketplaceEntryIsIntact() {
            ...
            foreach (object value in (object[])root["plugins"]) {
                ...
                if (plugin == null || Convert.ToString(plugin["name"]) != "codex-option-prompts") continue;
                IDictionary<string, object> source = (IDictionary<string, object>)plugin["source"];
                return Convert.ToString(source["path"]) == "./plugins/codex-option-prompts-old" &&
        ...
        public void ModifyInstalledSkill() {
            Write(Paths.PluginDirectory, "skills\\option-prompts\\SKILL.md", ModifiedText);
        }
        ...
        public bool ModifiedSkillStillExists() {
            string path = System.IO.Path.Combine(Paths.PluginDirectory, "skills", "option-prompts", "SKILL.md");
            return File.Exists(path) && File.ReadAllText(path) == ModifiedText;
        }

        public void Dispose() {
            string full = System.IO.Path.GetFullPath(Path);
            string safeRoot = System.IO.Path.GetFullPath(System.IO.Path.Combine(System.IO.Path.GetTempPath(), "CodexOptionPrompts.Tests"));
    ...
    internal static class InstallFixtures {
        public static MarketplacePlugin Entry() {
            return new MarketplacePlugin {
                Name = "codex-option-prompts",
                SourcePath = "./plugins/codex-option-prompts",
        ...
        public static bool IsCanonicalManagedEntry(IDictionary<string, object> root) {
            foreach (object value in (object[])root["plugins"]) {
                ...
                return Convert.ToString(source["source"]) == "local" &&
                    Convert.ToString(source["path"]) == "./plugins/codex-option-prompts" &&
