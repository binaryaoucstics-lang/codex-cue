...
namespace CodexCue.Tests {
    internal static class PluginInstallerTests {
        public static void Register(TestRegistry tests) {
            ...
            tests.Add("Installer migrates legacy plugin identity", delegate {
                using (TemporaryHome home = TemporaryHome.WithPrototype()) {
                    home.ConvertPrototypeToLegacy();
                    PluginInstaller.ForTest(home.Path).Install(home.CreateInstallRequest());
                    Assert.False(Directory.Exists(home.Paths.LegacyPluginDirectory));
                    Assert.True(Directory.Exists(home.Paths.PluginDirectory));
                    string marketplace = File.ReadAllText(home.Paths.MarketplacePath);
                    Assert.False(marketplace.Contains("codex-option-prompts"));
                    Assert.True(marketplace.Contains("codex-cue"));
                }
            });
