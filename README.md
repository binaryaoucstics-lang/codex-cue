...
一个轻量的 Windows 桌面提问向导。Codex 通过本机 MCP 工具打开逐题界面，用户可直接点击单选、多选，也可以回答开放式问题。插件可通过生命周期钩子把需要用户回答的问题自动路由到桌面窗口。
...
1. 打开仓库的 [Releases](https://github.com/binaryaoucstics-lang/codex-option-prompts/releases) 页面，下载：
   - `CodexOptionPrompts-Setup-x64.exe`
   - `SHA256SUMS.txt`
2. 在下载目录打开 PowerShell，校验安装包：

   ```powershell
   Get-FileHash .\CodexOptionPrompts-Setup-x64.exe -Algorithm SHA256
...
6. 可用以下命令验证安装：
   ...
   codex plugin list
   codex mcp list
   Get-CimInstance Win32_Process -Filter "Name='CodexOptionPrompts.exe'" |
     Select-Object ProcessId, ExecutablePath, CommandLine
   ```

   预期结果：
   - `codex-option-prompts@personal` 显示为 `installed, enabled`；
   - `codex_option_prompts` MCP 显示为 `enabled`；
   - 至少一个 `CodexOptionPrompts.exe --host` 常驻进程正在运行。
...
`CodexOptionPrompts-portable-x64.zip` 包含程序、完整插件、许可证和署名文件，不包含测试、PDB、Node.js 或 Python 运行时。

1. 从 [Releases](https://github.com/binaryaoucstics-lang/codex-option-prompts/releases) 下载 ZIP 和 `SHA256SUMS.txt` 并校验哈希。
2. 解压 ZIP，在解压目录运行：

```powershell
.\CodexOptionPrompts.exe --install-plugin
```

3. 按安装包教程的第 4～7 步信任钩子、新建任务并验证。

该命令会把程序和插件安装到当前用户目录。若只做协议集成，可直接使用 `plugins\codex-option-prompts\bin\CodexOptionPrompts.exe --mcp`，不需要后台 HTTP 服务。

### 升级与卸载

- 升级：下载新版本安装包并直接运行。安装器会先创建带时间戳和 SHA-256 清单的旧版备份，再替换托管文件。
- 卸载：从 Windows“设置 → 应用 → 已安装的应用”中卸载 **Codex Option Prompts**。卸载器会移除登录启动项，并在安全条件下恢复安装前备份。
...
- 程序安装在 `%LOCALAPPDATA%\Programs\CodexOptionPrompts`。
- 插件安装在 `%USERPROFILE%\plugins\codex-option-prompts`。
- 每次替换前会在 `%LOCALAPPDATA%\CodexOptionPrompts\backups` 创建带时间戳和 SHA-256 清单的备份。
...
- 没有看到选项窗口：新建 Codex 任务，然后用 `codex mcp list` 检查 `codex_option_prompts`，并确认任务管理器中存在带 `--host` 参数的 `CodexOptionPrompts.exe`。
- 插件未启用：运行 `codex plugin add codex-option-prompts@personal`，再新建任务。
- 宿主无响应：在任务管理器结束 `CodexOptionPrompts.exe`；下一次工具调用会自动重启宿主。
- 安装异常：查看 `%LOCALAPPDATA%\CodexOptionPrompts\install-status.json`，旧版文件仍保存在同目录的 `backups` 下。
