---
name: cue-prompts
description: Use whenever Codex is about to ask the user any question or needs any user response, including clarification, confirmation, approval, missing information, preferences, open-ended input, single-select choices, or multi-select choices. Route every such interaction directly through the codex_cue ask_options MCP tool instead of asking in assistant prose.
---

# Cue Prompts

1. Call the `ask_options` MCP tool from `codex_cue` immediately before every user-facing question. Do not first ask or list choices in assistant prose.
